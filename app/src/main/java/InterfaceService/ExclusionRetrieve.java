...
public class ExclusionRetrieve {
    ...
    //USED FOR NAV HOSPITAL LIST -- JJ
    public static void getHospInPatient(final Context context, final DatabaseHandler databaseHandler, final String memberId, final FragmentApiHospCallback fragmentApiHospCallback) {
        final AlertDialogCustom alertDialogCustom = new AlertDialogCustom();
        final ProgressDialog pd;
        pd = new ProgressDialog(context, R.style.MyTheme);
        pd.setCancelable(false);
        ...
        appInterface.getInpatientExclusions(memberId)
                ...
                .subscribe(new Subscriber<Exclusions>() {
                    ...
                    public void onNext(Exclusions exclusions) {
                        ...
                        getHospOutPatient(context, alertDialogCustom, memberId, fragmentApiHospCallback, pd, databaseHandler, exclusions);
                    }
                });

    }

    private static void getHospOutPatient(final Context context, final AlertDialogCustom alertDialogCustom, final String memberId, final FragmentApiHospCallback fragmentApiHospCallback, final ProgressDialog pd, final DatabaseHandler databaseHandler, final Exclusions exclusionsInPatient) {
        ...
        appInterface.getOutpatientExclusions(memberId)
                ...
                .subscribe(new Subscriber<Exclusions>() {
                    ...
                    @Override
                    public void onNext(Exclusions exclusions) {
                        setExcludedHosp(fragmentApiHospCallback, pd, databaseHandler, exclusionsInPatient , exclusions);
                    }
                });
    ...
    private static void setExcludedHosp(final FragmentApiHospCallback fragmentApiHospCallback, final ProgressDialog pd, final DatabaseHandler databaseHandler, final Exclusions exclusions, final Exclusions exclusions1) {
        final String[] excluded = exclusions.getExclusions();
        final String[] excluded1 = exclusions1.getExclusions();
        ...
        AsyncTask setExclusion = new AsyncTask() {
            ...
            protected void onPostExecute(Object o) {
                super.onPostExecute(o);
                fragmentApiHospCallback.onSuccess();
                pd.dismiss();
            }
            @Override
            protected void onPreExecute() {
