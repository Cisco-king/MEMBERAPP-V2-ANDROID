...
/**
 * Created by IPC on 12/19/2017.
 */

public class BasicTestLoaAdapter extends RecyclerView.Adapter<BasicTestLoaAdapter.Holder>  {
    private Context context;
    ArrayList<MaceRequest.GroupedByCostCenter.GroupedByDiag> arrayListGroupedByDiag = new ArrayList<>();

    public BasicTestLoaAdapter(Context context, ArrayList<MaceRequest.GroupedByCostCenter.GroupedByDiag> arrayListGroupedByDiag) {
    ...
    @Override
    public BasicTestLoaAdapter.Holder onCreateViewHolder(ViewGroup parent, int viewType) {
    ...
    @Override
    public void onBindViewHolder(BasicTestLoaAdapter.Holder holder, int position) {
        ...
        try {
            holder.tv_costcenter.setVisibility(groupedByDiag.isFirstInstance()? View.VISIBLE: View.GONE);
            if(groupedByDiag.isBasicTestFlag()){
        ...
        if (groupedByDiag.isFirstInstance()) {
            holder.tv_procedure_name.setText(groupedByDiag.getDiagType() == 2 ? "PRIMARY DIAGNOSIS:" : "OTHER DIAGNOSIS:");
...
//        holder.tv_approvalCode.setVisibility(groupedByDiag.getStatus().equals("PENDING")? View.GONE: View.VISIBLE);
        ...
        holder.tv_price_subtotal.setText(groupedByDiag.getSubTotal());

    }

    @Override
    public int getItemCount() { return arrayListGroupedByDiag.size();
    }
